...
func CompileToLLB(ctx context.Context, cfg *config.Config, variant string) (*llb.State, error) {
	...
	state, _, err := d2llb.Dockerfile2LLB(ctx, buffer.Bytes(), d2llb.ConvertOpt{})
...
func Compile(cfg *config.Config, variant string) (*bytes.Buffer, error) {
	...
	state, err := CompileToLLB(context.Background(), cfg, variant)
